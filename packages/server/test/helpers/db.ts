...
export async function resetOrgTables(): Promise<void> {
  await db.execute(
    sql`TRUNCATE TABLE organizations, memberships, teams, team_memberships, audit_logs, voids, void_access_grants, groups, tasks, task_assignees, checklist_items, comments, task_activities, invitations, notifications, tags, task_tags, group_tags RESTART IDENTITY CASCADE`,
  );
}
